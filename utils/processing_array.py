...
def give_index_points_below_threshold_on_axis(axis:str,threshold: float, points: np.ndarray):
    ...
    indices = np.where(points[:,axis_int] >= threshold)
...
def line_to_3Darray(line:np.ndarray,shape:Tuple[int,int])->np.ndarray:
    ...
    if np.shape(line)[0]*np.shape(line)[1]!=shape[0]*shape[1]*3:
        raise ValueError(f"Incorrect shape {shape} of line {line}")
...
def reduce_density_of_array(points: np.ndarray, colors: np.ndarray, densite: float):
...
def crop_array_from_zone_selection(points: np.ndarray, colors: np.ndarray,length: int=-1, height: int=-1, tableau_indice: Optional[List[int]] = []) -> Tuple[np.ndarray, np.ndarray, Optional[np.ndarray]]:
...
def filter_array_with_sphere_on_barycentre(points: np.ndarray, colors: np.ndarray, rayon: float, tableau_indice: Optional[List[int]] = []) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
...
if __name__ == '__main__':
    ...
    crop_array_from_zone_selection(points,colors,640,480)
